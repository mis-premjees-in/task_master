	...
	$lookups = [
		'madb' => [
			...
			'madb_who1' => [
				...
				'parent_from' => '`whos` LEFT JOIN `premises` as premises1 ON `premises1`.`premises_id`=`whos`.`whos_premise` ',
				'filterers' => [],
				'custom_query' => 'SELECT `whos`.`whos_id`, IF(CHAR_LENGTH(`whos`.`whos_who1`) || CHAR_LENGTH(`whos`.`whos_who2`) || CHAR_LENGTH(`whos`.`whos_description`), CONCAT_WS(\'\', `whos`.`whos_who1`, \'-\', `whos`.`whos_who2`, \'-\', `whos`.`whos_description`), \'\') FROM `whos` ORDER BY `whos`.`whos_id`',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false,
			],
