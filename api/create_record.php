...
//file_put_contents("insert_log.txt",json_encode("");
...
foreach($data as $key=>$value){
    if(str_contains($value,"data:image/jpg;base64")){
        $decoded = base64_decode(str_replace("data:image/jpg;base64","",$data[$key]));
        $file = date("YmdHis").rand(0,1000).".jpg";
        file_put_contents("../images/".$file, $decoded);
        $data[$key]=$file;
        //file_put_contents("insert.log",$decoded,FILE_APPEND);
    }
}
//insert the data into the table
$o = array('silentErrors' => true);
file_put_contents("insert_log1.txt","INSERT INTO {$table} (" . implode(',', array_keys($data)) . ") VALUES ('" . implode("','", array_values($data)) . "')");
