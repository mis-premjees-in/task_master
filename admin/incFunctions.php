	...
	*/
	...
	function get_parent_tables($table) {
		...
		$parents = [
			...
			'premises' => [
				'whens' => ['premises_closing', 'premises_opening'],
			],
			'pnb' => [
		...
		*/
		$lookupQuery = [
			'madb' => [
				...
				'madb_who3' => 'SELECT `whos`.`whos_id`, `whos`.`whos_who3` FROM `whos` LEFT JOIN `premises` as premises1 ON `premises1`.`premises_id`=`whos`.`whos_premise` ORDER BY 2',
			...
			'premises' => [
				'premises_opening' => 'SELECT `whens`.`whens_id`, IF(CHAR_LENGTH(`whens`.`whens_when2`) || CHAR_LENGTH(`whens`.`whens_when3`), CONCAT_WS(\'\', `whens`.`whens_when2`, \'-\', `whens`.`whens_when3`), \'\') FROM `whens` ORDER BY 2',
				'premises_closing' => 'SELECT `whens`.`whens_id`, IF(CHAR_LENGTH(`whens`.`whens_when2`) || CHAR_LENGTH(`whens`.`whens_when3`), CONCAT_WS(\'\', `whens`.`whens_when2`, \'-\', `whens`.`whens_when3`), \'\') FROM `whens` ORDER BY 2',
			],
			'pnb' => [
