...
// Data functions (insert, update, delete, form) for table howss

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function howss_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('howss');
	if(!$arrPerm['insert']) {
	...
	$recID = tableInsert('howss', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		howss_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function howss_copy_children($destination_id, $source_id) {
	global $Translation;
...
function howss_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('howss', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: howss_before_delete
	if(function_exists('howss_before_delete')) {
		$args = [];
		if(!howss_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	// child table: madb
	$res = sql("SELECT `howss_id` FROM `howss` WHERE `howss_id`='{$selected_id}'", $eo);
	$howss_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `madb` WHERE `madb_hows1`='" . makeSafe($howss_id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="madb_view.php?filterer_madb_hows1=' . urlencode($howss_id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
		$RetMsg = str_replace(['[<TableName>]', '<TableName>'], sprintf($childrenATag, 'madb'), $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = `howss_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = `howss_view.php?SelectedID=' . urlencode($selected_id) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		return $RetMsg;
	}

	sql("DELETE FROM `howss` WHERE `howss_id`='{$selected_id}'", $eo);

	// hook: howss_after_delete
	if(function_exists('howss_after_delete')) {
		$args = [];
		howss_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='howss' AND `pkValue`='{$selected_id}'", $eo);
}

function howss_update(&$selected_id, &$error_message = '') {
	global $Translation;
	...
	if($data['howss_hows1'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'How (S) 1 (Flowchart)': {$Translation['field not null']}<br><br>";
	...
	// hook: howss_before_update
	if(function_exists('howss_before_update')) {
		$args = ['old_data' => $old_data];
		if(!howss_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		['`howss_id`' => $selected_id],
		$error_message
	)) {
		echo $error_message;
		echo '<a href="howss_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('howss', $data['selectedID'], calculated_fields()['howss']);

	// hook: howss_after_update
	if(function_exists('howss_after_update')) {
		if($row = getRecord('howss', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['howss_id'];
		$args = ['old_data' => $old_data];
		if(!howss_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('howss', $selected_id);
}

function howss_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('howss', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('howss', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('howss', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('howss', $selectedId))) {
			return error_message($Translation['No records found'], 'howss_view.php', false);
		}
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/howss_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/howss_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'How (S) details', $templateCode);
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(howss_id)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%URLVALUE(howss_created)%%>', urlencode($urow['howss_created']), $templateCode);
		$templateCode = str_replace('<%%VALUE(howss_updated)%%>', safe_html($urow['howss_updated']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(howss_updated)%%>', urlencode($urow['howss_updated']), $templateCode);
	} else {
		$templateCode = str_replace('<%%VALUE(howss_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(howss_id)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(howss_hows1)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(howss_hows1)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(howss_hows2)%%>', '', $templateCode);
		...
		$templateCode = str_replace('<%%URLVALUE(howss_created)%%>', urlencode('<%%creationTimestamp%%>'), $templateCode);
		$templateCode = str_replace('<%%VALUE(howss_updated)%%>', '<%%editingTimestamp%%>', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(howss_updated)%%>', urlencode('<%%editingTimestamp%%>'), $templateCode);
	}
	...
	$rdata = $jdata = get_defaults('howss');
	if($hasSelectedId) {
		$jdata = get_joined_record('howss', $selectedId);
		if($jdata === false) $jdata = get_defaults('howss');
		$rdata = $row;
	}
	$templateCode .= loadView('howss-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: howss_dv
	if(function_exists('howss_dv')) {
		$args = [];
		howss_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
