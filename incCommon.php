		...
		$pcConfig = [
			...
			'utedb' => [
				'utedb_madb' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [0 => 'Utedb id', 1 => 'MADb Id', 2 => 'Premises Id', 3 => 'Utedb proof image', 4 => 'AI Audit (%) Score', 5 => 'PC Audit', 6 => 'RDA Audit', 7 => 'BB Audit', 8 => 'Customer Review(s)', 9 => 'CAR VIDEO', 10 => 'Collective TF', 11 => 'Delta Flag', 12 => 'Created AT', 13 => 'Updated AT'],
					'display-field-names' => [0 => 'utedb_id', 1 => 'utedb_madb', 2 => 'utedb_premises_id', 3 => 'utedb_proof_image', 4 => 'utedb_ai_audit', 5 => 'utedb_pc_audit', 6 => 'utedb_rda_audit', 7 => 'utedb_bb_audit', 8 => 'utedb_car', 9 => 'utedb_car_vid', 10 => 'utedb_col_tf', 11 => 'utedb_delta_flag', 12 => 'utedb_created', 13 => 'utedb_updated'],
					'sortable-fields' => [0 => '`utedb`.`utedb_id`', 1 => 2, 2 => 3, 3 => 4, 4 => '`utedb`.`utedb_ai_audit`', 5 => 6, 6 => 7, 7 => 8, 8 => '`utedb`.`utedb_car`', 9 => 10, 10 => 11, 11 => 12, 12 => '`utedb`.`utedb_created`', 13 => '`utedb`.`utedb_updated`'],
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					...
					'query' => "SELECT `utedb`.`utedb_id` as 'utedb_id', IF(    CHAR_LENGTH(`madb1`.`madb_id`) || CHAR_LENGTH(`whats1`.`whats_what1`), CONCAT_WS('',   `madb1`.`madb_id`, '-', `whats1`.`whats_what1`), '') as 'utedb_madb', IF(    CHAR_LENGTH(`premises1`.`premises_id`) || CHAR_LENGTH(`premises1`.`premises_name`), CONCAT_WS('',   `premises1`.`premises_id`, '-', `premises1`.`premises_name`), '') as 'utedb_premises_id', `utedb`.`utedb_proof_image` as 'utedb_proof_image', `utedb`.`utedb_ai_audit` as 'utedb_ai_audit', `utedb`.`utedb_pc_audit` as 'utedb_pc_audit', `utedb`.`utedb_rda_audit` as 'utedb_rda_audit', `utedb`.`utedb_bb_audit` as 'utedb_bb_audit', `utedb`.`utedb_car` as 'utedb_car', `utedb`.`utedb_car_vid` as 'utedb_car_vid', `utedb`.`utedb_col_tf` as 'utedb_col_tf', `utedb`.`utedb_delta_flag` as 'utedb_delta_flag', `utedb`.`utedb_created` as 'utedb_created', `utedb`.`utedb_updated` as 'utedb_updated' FROM `utedb` LEFT JOIN `madb` as madb1 ON `madb1`.`madb_id`=`utedb`.`utedb_madb` LEFT JOIN `whats` as whats1 ON `whats1`.`whats_id`=`madb1`.`madb_what1` LEFT JOIN `premises` as premises1 ON `premises1`.`premises_id`=`madb1`.`madb_premises_id` "
				],
			],
			'premises' => [
			],
