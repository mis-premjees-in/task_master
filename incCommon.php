	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'whos' => "`whos`.`whos_id` as 'whos_id', `whos`.`whos_who1` as 'whos_who1', `whos`.`whos_who2` as 'whos_who2', `whos`.`whos_who3` as 'whos_who3', `whos`.`whos_description` as 'whos_description', `whos`.`whos_swg_token` as 'whos_swg_token', `whos`.`whos_swg_email` as 'whos_swg_email', `whos`.`whos_created` as 'whos_created', `whos`.`whos_updated` as 'whos_updated'",
			...
			'pnb' => "`pnb`.`pnb_id` as 'pnb_id', `pnb`.`pnb_type` as 'pnb_type', IF(    CHAR_LENGTH(`premises1`.`premises_id`) || CHAR_LENGTH(`premises1`.`premises_name`), CONCAT_WS('',   `premises1`.`premises_id`, '-', `premises1`.`premises_name`), '') as 'pnb_premises_id', IF(    CHAR_LENGTH(`whos1`.`whos_who2`) || CHAR_LENGTH(`whos1`.`whos_who2`), CONCAT_WS('',   `whos1`.`whos_who2`, '-', `whos1`.`whos_who2`), '') as 'pnb_whos_id', `pnb`.`pnb_delta_flag` as 'pnb_delta_flag', IF(    CHAR_LENGTH(`whos2`.`whos_who2`) || CHAR_LENGTH(`whos2`.`whos_description`), CONCAT_WS('',   `whos2`.`whos_who2`, '-', `whos2`.`whos_description`), '') as 'pnb_elairda_id', `pnb`.`pnb_comments` as 'pnb_comments', `pnb`.`pnb_created` as 'pnb_created', `pnb`.`pnb_updated` as 'pnb_updated'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'howqs' => "`howqs` ",
			'howts' => "`howts` ",
			'utedb' => "`utedb` LEFT JOIN `madb` as madb1 ON `madb1`.`madb_id`=`utedb`.`utedb_madb` LEFT JOIN `whats` as whats1 ON `whats1`.`whats_id`=`madb1`.`madb_what1` LEFT JOIN `whos` as whos1 ON `whos1`.`whos_id`=`utedb`.`utedb_madb_who1` LEFT JOIN `whos` as whos2 ON `whos2`.`whos_id`=`utedb`.`utedb_elairda_id` LEFT JOIN `premises` as premises1 ON `premises1`.`premises_id`=`madb1`.`madb_premises_id` ",
			'premises' => "`premises` ",
		...
		$pcConfig = [
			...
			'utedb' => [
				'utedb_madb' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [0 => 'Utedb id', 1 => 'MADb Id', 2 => 'Who 1', 3 => 'Premises Id', 4 => 'Utedb proof image', 5 => 'AI Audit (%) Score', 6 => 'PC Audit', 7 => 'RDA Audit', 8 => 'BB Audit', 9 => 'Customer Review(s)', 10 => 'CAR VIDEO', 11 => 'Collective TF', 12 => 'Delta Flag', 13 => 'Utedb hows1', 14 => 'ELAI RDA ID', 15 => 'Created AT', 16 => 'Updated AT'],
					...
					'query' => "SELECT `utedb`.`utedb_id` as 'utedb_id', IF(    CHAR_LENGTH(`madb1`.`madb_id`) || CHAR_LENGTH(`whats1`.`whats_what1`), CONCAT_WS('',   `madb1`.`madb_id`, '-', `whats1`.`whats_what1`), '') as 'utedb_madb', IF(    CHAR_LENGTH(`whos1`.`whos_who2`) || CHAR_LENGTH(`whos1`.`whos_description`), CONCAT_WS('',   `whos1`.`whos_who2`, '-', `whos1`.`whos_description`), '') as 'utedb_madb_who1', IF(    CHAR_LENGTH(`premises1`.`premises_id`) || CHAR_LENGTH(`premises1`.`premises_name`), CONCAT_WS('',   `premises1`.`premises_id`, '-', `premises1`.`premises_name`), '') as 'utedb_premises_id', `utedb`.`utedb_proof_image` as 'utedb_proof_image', `utedb`.`utedb_ai_audit` as 'utedb_ai_audit', `utedb`.`utedb_pc_audit` as 'utedb_pc_audit', `utedb`.`utedb_rda_audit` as 'utedb_rda_audit', `utedb`.`utedb_bb_audit` as 'utedb_bb_audit', `utedb`.`utedb_car` as 'utedb_car', `utedb`.`utedb_car_vid` as 'utedb_car_vid', `utedb`.`utedb_col_tf` as 'utedb_col_tf', `utedb`.`utedb_delta_flag` as 'utedb_delta_flag', `utedb`.`utedb_hows1` as 'utedb_hows1', IF(    CHAR_LENGTH(`whos2`.`whos_who1`) || CHAR_LENGTH(`whos2`.`whos_who2`), CONCAT_WS('',   `whos2`.`whos_who1`, '-', `whos2`.`whos_who2`), '') as 'utedb_elairda_id', `utedb`.`utedb_created` as 'utedb_created', `utedb`.`utedb_updated` as 'utedb_updated' FROM `utedb` LEFT JOIN `madb` as madb1 ON `madb1`.`madb_id`=`utedb`.`utedb_madb` LEFT JOIN `whats` as whats1 ON `whats1`.`whats_id`=`madb1`.`madb_what1` LEFT JOIN `whos` as whos1 ON `whos1`.`whos_id`=`utedb`.`utedb_madb_who1` LEFT JOIN `whos` as whos2 ON `whos2`.`whos_id`=`utedb`.`utedb_elairda_id` LEFT JOIN `premises` as premises1 ON `premises1`.`premises_id`=`madb1`.`madb_premises_id` "
				],
				'utedb_madb_who1' => [
					'parent-table' => 'whos',
					'parent-primary-key' => 'whos_id',
					'child-primary-key' => 'utedb_id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Universal TEDB <span class="hidden child-label-utedb child-field-caption">(Who 1)</span>',
					'auto-close' => false,
					'table-icon' => 'table.gif',
					'display-refresh' => true,
					'display-add-new' => true,
